// app/(auth)/layout.tsx
//
// Shared shell for /sign-in, /sign-up, /forgot-password, and /sso-callback.
// - Desktop: video panel swaps sides depending on sign-in vs sign-up.
// - Mobile: video is always on top, form below.
// `/forgot-password` is treated the same as sign-in (video stays on the left)
// since it's conceptually part of the sign-in flow.
...
export default function AuthLayout({ children }: { children: ReactNode }) {
  ...
  // Only the sign-up route flips the layout; sign-in, forgot-password,
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-zinc-950 overflow-hidden">
      ...
      {/* FORM SIDE - Dynamic Soft Rounded Corners */}
      <div
        className={`flex-1 flex items-center justify-center p-4 sm:p-6 lg:p-12 
                    bg-zinc-900/70 backdrop-blur-3xl border-l border-white/10 
                    overflow-hidden relative
                    order-2 ${isSignUp ? "lg:order-1" : "lg:order-2"}
                    ${isSignUp 
                      ? "lg:rounded-r-3xl lg:rounded-tl-none" 
                      : "rounded-tl-3xl lg:rounded-l-3xl"}`}
      >
        <motion.div
