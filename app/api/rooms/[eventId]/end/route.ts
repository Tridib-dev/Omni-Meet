// app/api/rooms/[eventId]/end/route.ts
import { NextResponse } from "next/server";
import { endRoom } from "@/lib/actions/room.actions";
...
export async function POST(_request: Request, { params }: { params: RouteParams }) {
  try {
    ...
    const result = await endRoom(eventId.trim());
    const status = result.success ? 200 : result.reason === "unauthorized" ? 401 : 404;
    return NextResponse.json(result, { status });
  } catch (error) {
    console.error("[POST /api/rooms/[eventId]/end]", error);
